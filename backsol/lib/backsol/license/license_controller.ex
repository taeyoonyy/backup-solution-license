defmodule Backsol.LicenseController do
  ...
  def add(data, user) do
    IO.puts("/api/license/add with created")

    created =
      DateTime.utc_now()
      |> DateTime.add(3600 * 9, :second)
      |> to_string
      |> String.split(" ")
      |> Enum.at(0)

    data = string_to_atom(data) |> Map.put(:created, created)
  ...
  defp generate_license(data) do
    case data.type do
      "offline" ->
        ...
        encryption_license(
          data
          |> Map.merge(%{host: offHwData["host"], mac: offHwData["mac"]})
          |> Map.delete(:offKey)
        )
  ...
  defp encryption_license(data) do
    ...
    {cipher_text, auth_tag} =
      :crypto.crypto_one_time_aead(:aes_256_gcm, key, iv, message, aad, true)
